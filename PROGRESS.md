...
| 2c. Patch index (128×128, stride 64) with per-patch ship/water stats | DONE (committed and pushed); edge-aligned patches added, see step 10 |
| 3. Degradation engine (Gaussian / Poisson-Gaussian / correlated, SNR 0-40 dB, per-tile test noise) | DONE (committed and pushed); verified on real tiles, see steps 11 and 15 |
| 4. SQLite reference DB | DONE (committed and pushed); see steps 12 and 15 |
| 4b. Frozen protocol, statistics utilities, ship contrast-to-noise | DONE (committed and pushed); see step 14 |
| 4c. Evaluation harness (metrics, regions, per-ship metrics, results DBs, classical baselines) | DONE (committed and pushed); see step 16 |
| 5a. Dataset loader, model (NAFNet + FiLM + cross-band attention), loss, resumable training loop, tile prediction | DONE locally on CPU (committed and pushed); see step 17 |
...
### 6. Tile preparation (`scripts/prepare_tiles.py`, `src/data/`)
...
### 7. Findings that affect later design
...
### 8. Fold splits (`scripts/make_splits.py`, `src/data/splits.py`, `configs/splits.json`)
...
### 9. Risk validation (checked against the data on 2026-09-19)
...
### 10. Patch index (`scripts/make_patch_index.py`, `src/data/patches.py`)
...
### 11. Degradation engine (`src/degrade/`, `scripts/compute_signal_stats.py`, `scripts/check_degradation.py`)
...
- **Seeding:** `degradation_seed(base_seed, id, key)` uses SHA-256 (stable across processes and platforms, unlike Python's `hash`). **Superseded in step 14:** test noise is now generated per tile (not per patch) at 9 levels 0-40 dB with three noise types, and the training range is 5-40 dB.
...
### 12. Reference database (`db/schema.sql`, `src/data/refdb.py`, `scripts/build_db.py`)
`db/reference.sqlite` (git-ignored, rebuild with `python scripts/build_db.py`; the schema `db/schema.sql` is tracked). Foreign keys are enforced. **Table contents changed in step 14** (per-tile degradations, `peak` column, `tile_ship_contrast`); the DB is now 1.5 MB.
...
- **Results tables:** originally `runs` and `metrics` lived in this DB; **moved out in step 16** to separate results databases (`db/results_schema.sql`) so this DB stays purely derived and rebuildable.
...
- **Test noise per patch versus per tile (fixed in step 14).** The first design seeded noise per patch, so overlapping patches carried different noise on the same pixels, which rules out consistent full-tile noisy images, stitched evaluation and clean/noisy/reconstructed figures.
...
### 14. Changes made to strengthen the paper (2026-09-19)
All of these respond to step 13. Each was verified by a script. All work up to step 17 was committed and pushed on 2026-09-20 (`origin/Main` at `f934977`, pushed by `abhip-10`; 52 tracked files, no data). The unpushed commit that carried a Claude co-author trailer had its message rewritten locally before that push, so no commit made after the earlier request carries one; the five older pushed commits (before the earlier request) still do; removing it would need a history rewrite and force-push, which is on hold pending your decision.
...
### 15. Literature check on novelty (2026-09-19)
...
### 16. Evaluation harness and first reference results (2026-09-19)
...
- **Independent noise flatters denoisers by about 5–6 dB.** At 10 dB smoothing gains +10.2 dB on independent Gaussian noise but +4.5 dB on correlated noise (0 dB: +10.9 versus +5.1). This confirms the step 13 concern and justifies keeping `correlated_gaussian` in every comparison; results on independent noise alone would overstate any denoiser.
...
### 17. Loader, model, loss, training loop and tile prediction (2026-09-20, local CPU)
...
- **Not yet done at this point:** baseline architectures, a parameter-matched control, the evaluation driver (all done in step 18) and any GPU timing (still open).

### 18. Baseline architectures, parameter matching and the evaluation driver (2026-09-20, local CPU)
...
### 19. Kaggle GPU setup and pilot run (2026-09-23)
...
### 20. Full fold-0 training run and evaluation on Kaggle GPU (2026-09-23)
...
- Fold 1's test set is dominated by `rotterdam1` (49% of its test polygons, 61% of its ship pixels). Report per-tile results and not only per-fold averages (see step 9).
- Per-instance counting rule: resolved by `ship_instances.home_patch_id` (step 12). Evaluation code must use it and never sum `patches.ship_instances` over patches.
- Novelty: the web-search literature check (step 15) found no direct duplicate but strong precedent for each ingredient. Still to do by the user or with better access: manual Google Scholar/Scopus/IEEE searches with the queries listed in step 15, and reading the paywalled or blocked papers (RGT-YOLOv5Det, Real-ESRGAN on Sentinel-2A, floating-algae denoising) in full.
...
- Results storage: resolved in step 16. One results database per person or batch (`db/results_*.sqlite`, git-ignored), merged with `src/eval/results.py` (`merge` refuses duplicate run names). Run names must be unique across people, so use a prefix such as `<person>_<model>_fold<k>_seed<s>`.
- Invalid-pixel policy: `P_b` now excludes zeros and saturated pixels (done, step 11). Still open: whether to mask invalid pixels in SAM, ERGAS and the loss (probably yes), decided with the dataset/loss.
